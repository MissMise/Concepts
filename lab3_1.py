...
def equitable_bill_splitter():
   ...
   # Prompt the user to define how many people will be paying
   people = int(input("How many people are paying? "))
   #It is creating an empty list
   salaries = []
   #Will define the total of money from salaries from the people paying
   total = 0
   
   
   for i in range(people):
      #Prompst the user to define the salary for one individual
      sal = int(input("What is the salary of person {}?".format(i+1)))
      #Take the value of total and add sal to total
      total += sal
      #Add to the list sal
      salaries.append(sal)
   
   #Prompt the user to define the total of the bill
   total_bill = int(input("How much is the bill? "))
   
   for j in range(len(salaries)):
      #Loop through salaries
      #Calculate the share of the bill by salary
